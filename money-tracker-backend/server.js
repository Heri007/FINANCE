...
logger.info('✅ Routes Audit montées sur /api/audit');

app.use('/api/log-archive', logArchiveRoutes);
logger.info('✅ Routes Log Archive montées sur /api/log-archive');

// ✅ AJOUT 2 : Démarrer le CRON (optionnel, ou script séparé)
if (process.env.ENABLE_AUTO_ARCHIVE === 'true') {
  require('./scripts/cron-archive-logs');
  console.log('✅ Archivage automatique activé');
}
