// scripts/verify-argentliquide-integrity.js
...
async function verifyArgentLiquideIntegrity() {
  ...
  try {
    ...
    // ========================================
    const ArgentLiquideResult = await client.query(`
      SELECT id, name, type, balance, created_at, updated_at
      FROM accounts
      WHERE name = 'Argent Liquide'
    `);

    if (ArgentLiquideResult.rows.length === 0) {
      console.log('❌ Compte Argent Liquide introuvable !');
      return;
    }

    const argentliquide = ArgentLiquideResult.rows[0];
    const coffreId = argentliquide.id;
    const balanceStored = parseFloat(argentliquide.balance);

    console.log('📊 INFORMATIONS GÉNÉRALES');
    console.log('─────────────────────────────────────────────────────────────');
    console.log(`ID:              ${coffreId}`);
    console.log(`Type:            ${argentliquide.type}`);
    console.log(`Créé le:         ${new Date(argentliquide.created_at).toLocaleString('fr-FR')}`);
    console.log(`Dernière MAJ:    ${new Date(argentliquide.updated_at).toLocaleString('fr-FR')}`);
    ...
    for (const account of otherAccounts.rows) {
      ...
      const txFromAccount = await client.query(`
        ...
        WHERE account_id = $1
          ...
          AND type = 'expense'
          AND (
            description ILIKE '%argentliquide%'
            OR description ILIKE '%vers argentliquide%'
          )
      `, [account.id]);
