...
export default function BookkeeperDashboard({ onClose }) {
  ...
  useEffect(() => {
    console.log(
      "📊 BookkeeperDashboard - Transactions reçues:",
      transactions.length
    );
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (selectedAccountId) {
      list = list.filter(
        (t) => String(t.account_id) === String(selectedAccountId)
      );
    }
  ...
  <p className="text-[11px] text-blue-700 mt-1">
    Gain futur brut (hors transferts internes et dépenses).
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap gap-3 items-center">
              ...
              {(selectedAccountId || selectedProjectId || dateRange.from || dateRange.to) && (
                <button
                  onClick={() => {
