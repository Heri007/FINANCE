...
export function TransactionModal({ onClose, onSave, accounts }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (isNaN(finalAmount) || !formData.category || !formData.accountId || !formData.description) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all relative overflow-visible">
        <button type="button" onClick={onClose} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600 p-2 hover:bg-gray-100 rounded-lg">
          <X className="w-6 h-6" />
        </button>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`py-3 rounded-xl font-semibold transition-all ${
                  formData.type === "expense" ? "bg-rose-500 text-white shadow-lg" : "bg-gray-100 text-gray-600"
                }`}
              >
                Dépense
              </button>
              <button
                ...
                className={`py-3 rounded-xl font-semibold transition-all ${
                  formData.type === "income" ? "bg-emerald-500 text-white shadow-lg" : "bg-gray-100 text-gray-600"
                }`}
              >
                Revenu
              </button>
            ...
            {showCalculator && (
              <div className="absolute top-full right-0 mt-2 z-50 bg-white border-2 border-gray-200 rounded-xl shadow-xl p-3 w-64">
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <button 
                    type="button" 
                    onClick={() => { handleAmountBlur(); setShowCalculator(false); }}
