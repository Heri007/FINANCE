...
// ✅ MODAL memo (évite rerender si props inchangées)
const CategoryTransactionsModal = React.memo(function CategoryTransactionsModal({
  category,
  transactions,
  type,
  onClose,
  ...
  // pas de rendu si vide
  if (!Array.isArray(transactions) || transactions.length === 0) return null;

  // total en memo (transactions peut être gros)
  const total = useMemo(() => {
    let sum = 0;
    for (const tx of transactions) sum += Number(tx?.amount) || 0;
    return sum;
  }, [transactions]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col border-2 border-slate-300">
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <button
              onClick={onClose}
              className="p-2.5 hover:bg-slate-100 rounded-lg transition-all border-2 border-transparent hover:border-slate-300"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          <div className="space-y-2">
            {transactions.map((tx) => {
              const txDate = tx?.transactiondate || tx?.date || tx?.transaction_date;
              ...
              return (
                <div
                  ...
                  className="
                    flex justify-between items-center p-4 hover:bg-slate-50 rounded-xl cursor-pointer
                    transition-all duration-200 border-2 border-transparent hover:border-slate-200
                ...
                >
                  ...
                  <div className="text-right ml-4 flex-shrink-0">
                    ...
                    {!tx.is_posted && !tx.isposted && (
                      <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-700 bg-amber-100 border border-amber-300 px-2 py-0.5 rounded-md mt-1">
                        ⚠ Non validé
                      </span>
                    )}
                  </div>
...
// ✅ COMPOSANT PRINCIPAL (partie logique seulement)
export default function CategoryBreakdown({
  ...
  // au lieu de stocker tout l’objet {category,total,transactions} => on garde juste la clé
  const [selectedCategoryKey, setSelectedCategoryKey] = useState(null);

  // breakdown optimisé: 1 passe + Map
  const breakdown = useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      if (!entry) {
        entry = { category: cat, total: 0, transactions: [] };
  ...
  // total en memo basé sur breakdown (pas recalcul si transactions change mais breakdown identique)
  const total = useMemo(() => {
    let sum = 0;
    for (const c of breakdown) sum += c.total;
    return sum;
  }, [breakdown]);

  // catégorie sélectionnée dérivée
  const selectedCategory = useMemo(() => {
    if (!selectedCategoryKey) return null;
    return breakdown.find((b) => b.category === selectedCategoryKey) || null;
  }, [selectedCategoryKey, breakdown]);
    ...
    return (
      <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border-2 border-slate-200 p-8">
        ...
        <div className="flex flex-col items-center justify-center py-8">
          <div className="bg-slate-100 w-20 h-20 rounded-full flex items-center justify-center mb-4 border-2 border-slate-300">
  ...
  return (
    <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border-2 border-slate-200 overflow-hidden hover:shadow-xl transition-shadow duration-300">
      {/* Header fixe */}
      <div
        className={`p-6 pb-4 border-b-2 border-slate-200 bg-gradient-to-r ${
          ...
          {breakdown.map((item) => {
            ...
            return (
              <button
                key={item.category}
                type="button"
                onClick={() => setSelectedCategoryKey(item.category)}
                className="
                  w-full bg-slate-50 rounded-xl p-4 hover:bg-slate-100 transition-all duration-200
                  cursor-pointer text-left hover:shadow-md border-2 border-transparent hover:border-slate-200 group
                "
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <span className="font-bold text-slate-900 text-base truncate block">
                        {item.category}
                      </span>
                      <span className="text-xs text-slate-600 bg-slate-200 px-2 py-0.5 rounded-md font-semibold">
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className="flex items-baseline justify-end gap-2">
                      ...
                      <span className="text-sm font-bold text-slate-500 bg-slate-200 px-2 py-0.5 rounded-md">
                        {percentage}%
                      </span>
                    </div>
                  </div>
      ...
      {/* Footer fixe */}
      <div className="border-t-2 border-slate-200 p-6 pt-4 bg-gradient-to-r from-rose-50 to-pink-50">
        <div className="flex justify-between items-center">
