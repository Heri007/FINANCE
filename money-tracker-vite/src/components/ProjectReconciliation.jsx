...
export default function ProjectReconciliation({ projectId, onUpdate }) {
  ...
  const filteredLines = useMemo(() => {
    const lines =
      filterType === 'expense' ? projectLines.expenses : projectLines.revenues;
    return lines.filter(
      (l) =>
        searchTerm === '' ||
  ...
  const handleLink = async (lineId) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setUnlinkedTxs((prev) =>
          prev.filter((t) => t.transaction_id !== selectedTx.transaction_id)
        );
        setSelectedTx(null);
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 rounded-xl overflow-hidden border border-gray-200">
      ...
      <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="border-r border-gray-200 flex flex-col bg-white">
          <div className="p-3 bg-gray-50 text-xs font-bold text-gray-500 uppercase tracking-wider border-b flex justify-between">
            ...
            <span className="bg-gray-200 px-2 rounded-full text-gray-700">
              {filteredTxs.length}
            </span>
          </div>
            ...
            {filteredTxs.map((tx) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`font-bold text-sm whitespace-nowrap ${tx.type === 'expense' ? 'text-red-600' : 'text-green-600'}`}
                  >
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {filteredLines.map((line) => {
              const projected = parseFloat(
                line.projected_amount || line.projectedAmount || 0
              );
              ...
              const diff = selectedTx
                ? Math.abs(projected - parseFloat(selectedTx.amount))
                : 0;
      ...
      <div className="bg-white border-t p-2 text-center text-xs text-gray-400">
        Sélectionnez une transaction à gauche, puis cliquez sur "Lier" sur la ligne
