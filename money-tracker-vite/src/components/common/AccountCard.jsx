...
export function AccountCard({ account, onDelete, onClick }) {
  ...
  return (
    ...
    >
      ...
      {/* Ligne accent top */}
      <div
        className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${style.accentLine} opacity-60`}
      ></div>
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div
              className={`p-1.5 ${style.iconBg} backdrop-blur-sm rounded-lg border ${style.border} flex-shrink-0`}
            >
              <IconComponent className={style.iconColor} size={16} strokeWidth={2.5} />
            </div>
            <div className="min-w-0 flex-1">
              <h3
                className={`font-bold text-lg truncate leading-tight ${style.textColor}`}
              >
                {account.name}
              </h3>
              <p
                className={`text-[9px] uppercase tracking-wider font-semibold ${style.textColor} opacity-70`}
              >
                {account.type}
              </p>
            </div>
        ...
        <div className="pt-2 border-t border-white/20">
          <div className="flex items-baseline justify-between">
            <span
              className={`text-[10px] uppercase tracking-widest font-bold ${style.textColor} opacity-60`}
            >
              Solde
            </span>
            <p
              className={`text-xl font-black leading-none ${
                account.balance >= 0 ? style.textColor : 'text-red-300'
              }`}
            >
