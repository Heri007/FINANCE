...
const cashFlowData = useMemo(() => {
  ...
  // ✅ Créer une date locale à partir de startDate
const today = new Date(startDate);
today.setHours(0, 0, 0, 0);

// ✅ Convertir en string ISO SANS utiliser toISOString (qui convertit en UTC)
const todayStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;


  const realBalanceToday = balance || 0;

  console.log('💰 Solde Coffre actuel:', realBalanceToday);
  console.log('📅 Date de référence (aujourd\'hui):', todayStr);
  // ✅ Debug avec date locale ISO
const debugStartDate = new Date(startDate);
console.log('startDate (ISO):', `${debugStartDate.getFullYear()}-${String(debugStartDate.getMonth() + 1).padStart(2, '0')}-${String(debugStartDate.getDate()).padStart(2, '0')}`);
    ...
    const days = []; // ✅ Doit être déclaré AVANT la boucle while
    ...
    while (current <= end) {
      // ✅ Créer dateStr à partir des composantes locales
...
// ✅ Utiliser dateStr au lieu de currentDate.toLocaleDateString()
if (currentDate.getTime() === today.getTime()) {
  console.log('🔍 === TRANSACTIONS PRÉVISIONNELLES AUJOURD\'HUI ===');
  console.log('Total:', plannedTransactions.length);
...
plannedTransactions.forEach((tx, index) => {
  ...
  // ✅ LOG seulement pour aujourd'hui (pas les 3 premières de chaque jour)
  const isToday = currentDate.getTime() === today.getTime();
  if (isToday) {
    console.log('✅ Transaction prév. ACCEPTÉE (index', index, '):', {
      dateFromTx: txDateStr,
      currentDateISO: dateStr,
      type: tx.type,
      amount: tx.amount,
  ...
  // ✅ CORRECTION: Inclure aujourd'hui dans les prévisions
